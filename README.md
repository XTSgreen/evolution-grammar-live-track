...
然后在预测期结束后用公开数据对账。仓库中**不包含预测系统的源代码**，只有预测结果
（`tools/` 内仅含月度对账进度脚本，`.github/` 内仅含其调度配置）。
...
GitHub Actions（`monthly_update.yml`）每月 1 日运行：下载最新 Nextstrain ncov open 元数据，
统计预测窗口内已首次出现的 RBD 突变，对照 v1/v2 的 top-10 / top-50 提名累计命中，
追加写入 `reconciliation/progress_YYYY-MM-DD.json`。进度快照只追加、不修改注册内容，
不构成正式对账结果；正式对账于 2027-03-01 按上述规则执行。
